...
trait Tables {
   ...
   *  @param status Database column STATUS SqlType(CHAR), Length(3,false)
  ...
  final case class UsersRow(userId: String, userName: String, email: String, password: String, status: String, createdAt: java.sql.Timestamp, updatedAt: java.sql.Timestamp, versionNo: Int)
  /** GetResult implicit for fetching UsersRow objects using plain SQL queries */
  implicit def GetResultUsersRow(implicit e0: GR[String], e1: GR[java.sql.Timestamp], e2: GR[Int]): GR[UsersRow] = GR{
    prs => import prs._
    UsersRow.tupled((<<[String], <<[String], <<[String], <<[String], <<[String], <<[java.sql.Timestamp], <<[java.sql.Timestamp], <<[Int]))
  }
  /** Table description of table USERS. Objects of this class serve as prototypes for rows in queries. */
  class Users(_tableTag: Tag) extends profile.api.Table[UsersRow](_tableTag, Some("user_manager"), "USERS") {
    def * = (userId, userName, email, password, status, createdAt, updatedAt, versionNo) <> (UsersRow.tupled, UsersRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(userId), Rep.Some(userName), Rep.Some(email), Rep.Some(password), Rep.Some(status), Rep.Some(createdAt), Rep.Some(updatedAt), Rep.Some(versionNo)).shaped.<>({r=>import r._; _1.map(_=> UsersRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column STATUS SqlType(CHAR), Length(3,false) */
    val status: Rep[String] = column[String]("STATUS", O.Length(3,varying=false))
    ...
    /** Index over (email) (database name EMAIL_INDEX) */
    val index1 = index("EMAIL_INDEX", email)
  }
