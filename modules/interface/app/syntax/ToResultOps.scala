...
trait ToResultOps extends Results with ToFutureOps with ToEitherOps {
  ...
  implicit class ErrorToOps(error: Error) {
    def toResult: mvc.Result = {
      if (error.logging) logger.error(error.toString)
  ...
  implicit class ResultToResultOpts[A](result: Result[A]) {
    ...
    private def handleErrorAsync(fun: A => mvc.Result)(implicit ec: ExecutionContext): Future[mvc.Result] = {
    ...
    private def handleErrorSync(fun: A => mvc.Result): mvc.Result = {
